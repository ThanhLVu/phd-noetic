...
void calculateTransformationMatrix()
{
    ...
    if (base_to_qr_flag && depth1_to_qr_flag && cam1_to_depth1_flag)
    {
        ...
        tf2::Matrix3x3 temp_rot;
        double temp_R, temp_P, temp_Y;
        unsigned int temp_no_sol;
        temp_rot.setRotation(base_to_cam1_matrix.getRotation());
        temp_rot.getRPY(temp_R, temp_P, temp_Y, temp_no_sol);
        std::cout << "==> Rotataion RPY: " << temp_R << " " << temp_P << " " << temp_Y << ". No. sol: " << temp_no_sol << std::endl;
        
        broadcastTransform(base_to_cam1_matrix, "base_link", "camera1_link");
        cam1_done = true;
    }

    ROS_INFO("Calibrating cam 2");
    ...
    if (base_to_qr_flag && depth2_to_qr_flag && cam2_to_depth2_flag)
    {
        ...
        double temp_R, temp_P, temp_Y;
        unsigned int temp_no_sol;
        temp_rot.setRotation(base_to_cam2_matrix.getRotation());
        temp_rot.getRPY(temp_R, temp_P, temp_Y, temp_no_sol);
        std::cout << "==> Rotataion RPY: " << temp_R << " " << temp_P << " " << temp_Y << ". No. sol: " << temp_no_sol << std::endl;
