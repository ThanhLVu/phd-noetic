...
const double ARM_MIN_RANGE = 0.3;
...
Eigen::Matrix4d getTransformationMatrix(geometry_msgs::TransformStamped input_trans)
{
    Eigen::Vector3d translation(input_trans.transform.translation.x, input_trans.transform.translation.y, input_trans.transform.translation.z);
    Eigen::Quaterniond rotationQuaternion(input_trans.transform.rotation.w, input_trans.transform.rotation.x, input_trans.transform.rotation.y, input_trans.transform.rotation.z);
...
void combinePointclouds()
{
    ...
    Eigen::Matrix4d transform = getTransformationMatrix(transformStamped);
...
void removeArm(std::vector<singleCluster> &single_cluster, std::vector<bool> &validation)
{
    Eigen::Matrix4d transform = getTransformationMatrix(transformStamped2);

    Eigen::Vector4d relative_position, original_position;
    
    validation.clear();
    validation.resize(single_cluster.size());
    for(int i=0; i<validation.size(); i++)
    {
        validation.at(i) = true;
    }
    ...
    for(int unique_index=0; unique_index<single_cluster.size(); unique_index++)
    {
        for(int i=0; i<8; i++)
        {
            original_position << single_cluster.at(unique_index).bounding_box.at(i).x, left_cam_cloud->points.at(i).y, left_cam_cloud->points.at(i).z, 1.0; 
            relative_position = transform*original_position;
            
            if(relative_position(0) < ARM_MIN_RANGE) 
            {
                validation.at(unique_index) = false;
                break;
            }
        }
    }
...
// Visualize the bounding boxes in world frame
void drawWorldView(std::vector<singleCluster> &single_cluster, std::vector<bool> &validation)
{
...
void publishMarkersMsg(std::vector<singleCluster> &single_cluster, std::vector<bool> &validation)
{
...
void cloudProcess()
{
    ...
    std::vector<bool> box_validation;
    removeArm(single_cluster_vec, box_validation);
    // drawWorldView(single_cluster_vec, box_validation);
    publishMarkersMsg(single_cluster_vec, box_validation);
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "combine_cameras_sim_arm_removed");
    ...
    while (ros::ok())
    {
        ...
        // Get transform from right camera to base_link  (from the urdf model)
        try
        {
            // Change link name if needed
            transformStamped2 = tfBuffer.lookupTransform("base_link", "right_camera_depth_optical_frame", ros::Time(0));
        }
        catch (tf2::TransformException &ex) {
            ROS_WARN("%s", ex.what());
            ros::Duration(1.0).sleep();
