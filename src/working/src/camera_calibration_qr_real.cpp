...
geometry_msgs::Pose depth_to_qr_pose;
geometry_msgs::TransformStamped cam_to_depth_transform;
// Flag values to prevent unnecessary variable update
bool status_flag = false;
bool base_to_qr_flag = false;
bool cam_to_qr_flag = false;
bool depth_to_cam_flag = false;
...
void qrPosCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    if(!cam_to_qr_flag && status_flag)
    {
        ...
        transform_broadcast.header.frame_id = "camera_depth_optical_frame";
        transform_broadcast.child_frame_id = "visp_qr_frame";
        transform_broadcast.transform.translation.x = depth_to_qr_pose.position.x;
        transform_broadcast.transform.translation.y = depth_to_qr_pose.position.y;
        transform_broadcast.transform.translation.z = depth_to_qr_pose.position.z;

        transform_broadcast.transform.rotation.x = depth_to_qr_pose.orientation.x;
        transform_broadcast.transform.rotation.y = depth_to_qr_pose.orientation.y;
        transform_broadcast.transform.rotation.z = depth_to_qr_pose.orientation.z;
        transform_broadcast.transform.rotation.w = depth_to_qr_pose.orientation.w;
...
void calculateTransformationMatrix()
{
    if (base_to_qr_flag && cam_to_qr_flag)
    {
        ...
        tf2::Transform depth_to_qr_matrix;

        Eigen::Vector3d depth_to_qr_trans(depth_to_qr_pose.position.x, depth_to_qr_pose.position.y, depth_to_qr_pose.position.z);
        Eigen::Quaterniond depth_to_qr_rot(depth_to_qr_pose.orientation.w, depth_to_qr_pose.orientation.x, depth_to_qr_pose.orientation.y, depth_to_qr_pose.orientation.z);

        generateTransformationMatrix(depth_to_qr_trans, depth_to_qr_rot, depth_to_qr_matrix);

        std::cout << depth_to_qr_trans << std::endl;
        std::cout << depth_to_qr_rot.w() << " " << depth_to_qr_rot.x() << " " << depth_to_qr_rot.y() << " " << depth_to_qr_rot.z() << std::endl;

        // Process cam_to_depth transform from tf_tree
        tf2::Transform cam_to_depth_matrix;

        Eigen::Vector3d cam_to_depth_trans(cam_to_depth_transform.transform.translation.x, cam_to_depth_transform.transform.translation.y, cam_to_depth_transform.transform.translation.z);
        Eigen::Quaterniond cam_to_depth_rot(cam_to_depth_transform.transform.rotation.w, cam_to_depth_transform.transform.rotation.x, cam_to_depth_transform.transform.rotation.y, cam_to_depth_transform.transform.rotation.z);

        std::cout << cam_to_depth_trans << std::endl;
        std::cout << cam_to_depth_rot.w() << " " << cam_to_depth_rot.x() << " " << cam_to_depth_rot.y() << " " << cam_to_depth_rot.z() << std::endl;

        generateTransformationMatrix(cam_to_depth_trans, cam_to_depth_rot, cam_to_depth_matrix);

        // Calculate base_to_cam transform
        tf2::Transform base_to_cam_matrix;
        base_to_cam_matrix = base_to_qr_matrix.inverse() * depth_to_qr_matrix.inverse() * cam_to_depth_matrix;
        ...
        broadcastTransform(base_to_cam_matrix, "base_link", "camera_link");
        resetFlags();
    }
}
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        if(!depth_to_cam_flag)
        {
            try
            {
                // Change link name if needed
                cam_to_depth_transform = tfBuffer.lookupTransform("camera_depth_optical_frame", "camera_link", ros::Time(0));
            }
            catch (tf2::TransformException &ex) {
                ROS_WARN("%s", ex.what());
                ros::Duration(1.0).sleep();
            ...
            depth_to_cam_flag = true;
            ROS_INFO("Got cam_to_depth information");
        }
